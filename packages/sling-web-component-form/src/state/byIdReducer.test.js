...
describe('byIdReducer', () => {
  ...
  it('Should set username used to true', () => {
    state = byIdReducer(state, updateFieldUsed('username', true));
    expect(state).to.eql({
      [FORM]: {
        error: null,
        isValidating: false,
  ...
  it('Should remove username field', () => {
    state = byIdReducer(state, removeField('username'));
    expect(state).to.eql(INITIAL_STATE);
  });
