...
describe('atLevel', () => {
  ...
  it('Should execute the validator function with a value', () => {
    ...
    atLevel(wrapperFn)(validatorFn, 'some value');
    expect(validatorFn).to.have.been.calledOnceWith('some value');
  });
...
describe('validate', () => {
  ...
  it('Should dispatch finishValidation if ' +
    ...
    const validatorThunk = () => 'some message';
    ...
    expect(finish).to.have.been.calledOnceWith('name', 'some message');
  });

  it('Should dispatch both startValidation and finishValidation if ' +
    'validation returns a Promise', async () => {
    const validator = Promise.resolve('some message');
